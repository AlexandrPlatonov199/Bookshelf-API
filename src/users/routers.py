...
from src.users.auth import authenticate_user, create_jwt_token, get_password_hash
...
async def auth_register(user_data: SUser):
    ...
    await UserDAO.add(
        username=user_data.username, email=user_data.email, hash_password=hash_password
    )
